...
for t, thr, thrIP in zip(thrList, thrPercList, thrIPList):
    ...
    for subject in subjectIDs[:nSubj]:
        ...
        for i in range(iterPerSubj):
            scores, kParsed = upperBoundAttackIP(dataDir, subject, thrIP, kStart, kEnd, nK)
            itScPth = f'{subjDir}/iter_{i}_subScores_{subject}.txt'
            itKPth = f'{subjDir}/iter_{i}_subParsedK_{subject}.txt'
            np.savetxt(itScPth, scores, delimiter=',', fmt='%.4e')
